...
void drawConfirmSaleMenuScreen(MenuContext *mc) {
    ...
    sprintf(
            purchaseText,
            "You sold %s (%d)",
            mc->itemToSell.item->name,
            mc->quantity);
    drawInMenu(b, purchaseText);
    char purchaseText2[64];
    sprintf(purchaseText2, "for %d coins.", mc->quantity * mc->itemToSell.item->worth);
    drawInMenu(b, purchaseText2);
}

MenuSelectResponse *confirmSaleMenuItemSelected(MenuContext *mc) {
    for (int i = 0; i < mc->quantity; i++) {
        removeItem(mc->player, mc->itemToSell.item);
    }
    mc->player->coins += mc->itemToSell.item->worth * mc->quantity;
